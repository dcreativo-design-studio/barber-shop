...
function BookingCalendar() {
  ...
  // Gestione caricamento iniziale con fade-in
  useEffect(() => {
  ...
  // Spinner durante il caricamento iniziale
  if (initialLoading) {
    return (
  ...
  return (
    <div className={`max-w-4xl mx-auto px-4 transition-opacity duration-300 ${contentVisible ? 'opacity-100' : 'opacity-0'}`}>
      {/* Form prenotazione */}
      <div className="bg-[var(--bg-secondary)] p-6 rounded-lg shadow-xl mb-8">
        ...
        {error && (
          <div className="bg-red-500/20 border border-red-500 text-red-500 p-3 rounded mb-4 error-message">
            {error}
          </div>
        )}

        {success && (
          <div className="bg-green-500/20 border border-green-500 text-green-500 p-3 rounded mb-4 animate-fadeInSlideDown">
            {success}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Barbiere */}
          <div>
            <label className="block text-[var(--accent)] mb-2 font-medium">Barbiere</label>
            <select
              value={selectedBarber}
              onChange={(e) => setSelectedBarber(e.target.value)}
              required
              className="w-full p-3 rounded bg-[var(--bg-primary)] text-[var(--text-primary)] border border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)] transition-all"
            >
          ...
          {/* Servizio */}
          <div>
            <label className="block text-[var(--accent)] mb-2 font-medium">Servizio</label>
            <select
              value={selectedService}
              onChange={(e) => setSelectedService(e.target.value)}
              required
              disabled={!selectedBarber}
              className="w-full p-3 rounded bg-[var(--bg-primary)] text-[var(--text-primary)] border border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)] transition-all disabled:opacity-50"
            >
          ...
          {/* Data */}
          <div>
            <label className="block text-[var(--accent)] mb-2 font-medium">Data</label>
            <input
              type="date"
              min={minDate}
              max={maxDate}
          ...
          {selectedDate && selectedService && selectedBarber && (
            <div>
              <label className="block text-[var(--accent)] mb-2 font-medium">Orario</label>
              <TimeSlots
          ...
          <button
            ...
            className="w-full bg-[var(--accent)] text-white font-bold py-3 px-4 rounded transition-all duration-300 hover:opacity-90 disabled:opacity-50 button-3d"
          >
        ...
        <div className="bg-[var(--bg-secondary)] p-6 rounded-lg shadow-xl">
          ...
          <div className="space-y-4">
            {userAppointments.map(appointment => {
              ...
              return (
                <div key={appointment._id} className="bg-[var(--bg-primary)] p-4 rounded-lg flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 hover:shadow-lg transition-all duration-200">
                  <div>
